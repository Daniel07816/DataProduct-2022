...
ggplot(dat3, aes(x = Customer.ID, y = profit)) +
  geom_col() +labs(title="Clientes que generan mas ganancias", 
       subtitle="-", 
       caption="source: tienda.csv dataset") + 
...
# Mejor y Peor Producto. Analisis p/segm.
...
### Envios Canon imageCLASS 2200 Advanced Copier
...
### Envios Cubify CubeX 3D Printer Triple Head Print
...
my_data %>%
  ...
  hc_title(text = "<b>Cantidad de productos por segmento")
```
...
my_data %>%
  ...
  hc_title(text = "<b>Ventas totales por segmento")
```
...
```{r Data}
my_data %>% head(100) %>% datatable(class = "compact",caption = "Data manejada en el proyecto", width = 800)
```
